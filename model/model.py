...
class Model:
    ...
    def getTop3Archi(self):
         return sorted(self._graph.edges(data=True), key=lambda x: x[2]["weight"], reverse=True)[:3] #dal più grande al più piccolo ordinati secondo il peso, i primi tre

    def getConnessaInfo(self):
        components = list(nx.connected_components(self._graph))
        largest = max(components, key=len)
        subgraph= self._graph.subgraph(largest).copy() #sottografo ottenuto dai nodi di dim maggiore
        orderedNodes= sorted(subgraph.nodes(), key=lambda n: self._graph.degree(n), reverse=True) #ordino i nodi in base al grado
        details= [(n,self._graph.degree(n)) for n in orderedNodes] # lista di tuple nodo-grado
        return len(components), largest, details

    def getListaPilotiOttima(self, k):
        self._optListPiloti = []
        self._minDistGiorni = 100*365 #upper bounds

        components =list(nx.connected_components(self._graph))

        if len(components) < k: #allora non ho abbastanza componenti connesse da cui pescare e non posso trovare una soluzione
            return None, 0
        parziale = []
    ...
    def _ricorsione(self, components, k, parziale, indexComponente):
        ...
        #1)esco se l'indice, che indica quale comp connessa sto considerando a questa iterazione,
        #è diventato maggiore o uguale al numero di componenti connesse totali, perchè vuol dire che non ho altre componenti da cui pescare
        ...
        #Se non sono uscito allora posso aggiungere ancora piloti. Per questa componente, di indexComponente,
        #provo a ingaggiare un pilota oppure a non ingaggiare nessuno

        #caso1, inserisco un pilota appartenente a questa componente connessa. In questo branch provo tutti i piloti che fanno parte della componente connessa in esame
        ...
        #caso2, mi tengo un branch di esplorazione in cui io non ho preso proprio nessuno da questa componente
        self._ricorsione(components, k, parziale, indexComponente+1)
